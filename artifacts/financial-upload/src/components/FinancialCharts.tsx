...
// ── Ratio interpretation lookup ───────────────────────────────────────────────

interface RatioMeta {
  description: string;       // one-line meaning
  benchmark: (v: number) => string; // contextual interpretation
}
...
function getRatioMeta(label: string): RatioMeta | null {
...
function GaugeItem({ metric, index }: { metric: KeyMetric; index: number }) {
  ...
  const meta = getRatioMeta(metric.label);
